...
他に PERF001(blog)、CORRECT002(blog詳細)、ARCH002(product)、SEC002(contact)、
og:image/og:title等の細かいSEO系warning/infoが多数続く(SEC001は `svelte-vitals.config.ts` の
設定でデフォルト無効化済み — 詳しくは次のセクション)。詳しくは下の「サイト構成」を参照。
...
`svelte-vitals.config.ts` は `rules: { SEC001: 'off' }` を有効化した状態で入っている
(product ページの `{@html}` は変わらずソースに残っているが、config でこのルールだけ無効化している
ので `pnpm scan` には出てこない)。他のオプションはコメントアウトされた例として入っている:

```ts
export default defineConfig({
	// treatDynamicAs: 'pass', // 'pass' | 'warn' | 'fail'
	// metaComponents: ['Seo'],
	rules: { SEC001: 'off' }, // 有効化済み — この行をコメントアウトして pnpm scan を再実行すると SEC001 が復活する
	// failOn: 'critical',
	// weights: {} // e.g. { seo: 2 } — 有効化すると Health が動くのを確認できる(seo は現状最も低いカテゴリ)
});
```

`--rules`/`--ignore`/`--weights` などのCLIフラグでも同じことをその場で試せる
(`pnpm exec svelte-vitals --ignore SEC001`)。CLIフラグはconfigより優先される。
...
SEO003, SEO008, PERF001 など)に対して付く。このページにも `{@html}` があるが、`main` 側の
`svelte-vitals.config.ts`(`rules: { SEC001: 'off' }`)がPRのマージコミットにも適用されるため
SEC001としては出てこない — config はブランチをまたいで効くことの実例にもなっている。`main` 側の
既存の問題は `diff`/`baseline` が `origin/main` 基準のため出てこない — CIゲートは
「このPRが新しく持ち込んだ問題」だけに反応する。
...
| `/products/[id]`                              | `{@html}` が未サニタイズ(SEC001。config でデフォルト無効化 — 試すには config から外す)、`ProductInfo` コンポーネントが11個のprops(ARCH002) |
